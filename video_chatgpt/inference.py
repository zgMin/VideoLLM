...
def video_chatgpt_infer(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len):
    ...
    question_ids = torch.as_tensor(question_ids.input_ids).cuda()
...
def video_chatgpt_infer(video_frames, video_features, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len):
    """
    Run inference using the Video-ChatGPT model.
    ...
    if video_features is None:
        ...
        video_spatio_temporal_features = video_features.cuda()
...
def video_chatgpt_infer_cd(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len):
    """
    Run inference using the Video-ChatGPT model.
    ...
    # 查找video_token的位置
    vid_patch_id = tokenizer.convert_tokens_to_ids(DEFAULT_VIDEO_PATCH_TOKEN)
    vid_start_id = tokenizer.convert_tokens_to_ids(DEFAULT_VID_START_TOKEN)
    vid_end_id = tokenizer.convert_tokens_to_ids(DEFAULT_VID_END_TOKEN)
    patch_mask = input_ids == vid_patch_id
    start_mask = input_ids == vid_start_id
    end_mask = input_ids == vid_end_id
    # 将三个掩码合并
    video_mask = (patch_mask | start_mask | end_mask).int()
...
def video_chatgpt_infer_cd(video_frames, video_features, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len):
    """
    Run inference using the Video-ChatGPT model.
